var availSCont = '<div class="%cat2%"><p class="description">%description%</p><div class="row"><div class="col-sm-2"><input type="checkbox" name="article_id" value="%article_id%" onclick="processRequest.call(%cat%, $(this), this);"></div><div class="col-sm-7 heading">%heading%</div><div class="col-sm-3 author">%author%</div></div></div>';
var chkdSCont = '<li><span class="close-diag close-diagA" onclick="removeRequestFromFault.call(%cat%, $(this), this); simulateClick(this.getAttribute(\'value\'), \'%cat2%\')"; value="%article_id%">&times;</span><span>%description%</span></li>';
var faultDisp = '<div class="fault-display col-sm-5" onclick="show_a_fault($(this));" id="%sid%"><div class="description">%description%</div><div class="row hAD"><div class="heading col-sm-8">%heading%</div><div class="col-sm-4"><div class="author">%author%</div><div class="hr"></div><div class="date">%date%</div></div></div></div>';
var aFault = '<header><h4 class="heading">%heading%<span class="close-diag close-diag2" onclick="hide_a_fault()">&times;</span></h4></header><section class="description"> %description%</section><section class="symptoms"> <h4>Symptoms</h4> <ul class="col-sm-11 col-sm-offset-1">%symptom% </ul></section><section class="solutions"> <h4>Solutions</h4> <ul class="col-sm-11 col-sm-offset-1">%solution% </ul></section><section class="publishInfo">This fault was added <span class="date">%date%</span> by <span class="author">%author%</span></section>';
var sympInAFault = ' <li><span class="symptom">%heading%: </span>%description%</li>';
var solnInAFault = ' <li><span class="solution">%heading%: </span>%description%</li>';
...
function createInput() {
    ...
    $('<button type="button" id="searchDBMoreBtn" style="float:right;">Show More</button>').appendTo("main");
}
...
function processDate(dateToProcess) {
    var differenceTotalSecs, differenceTimeMilliS, differenceDays, differenceHours, differenceMins, differenceSecs, countdownText, countdownText2;
    ...
    dateToProcessF2 = new Date(arrD[0], newMonth, arrD[2], arrD[3], arrD[4], arrD[5], arrD[6]);
...
function getFaults() {
    $.post($("#addFaultForm").attr("action"), { formID: "getFaults" }, function(info) {
...
function getRequests(formID) {
    var text = "",
        ...
        cat2 = "";
    if (formID === "gSymp") {
        text = "No symptom has been added yet, use the add symtom below to add one.";
        location = "#availSymp";
        cat = "sympToAdd";
        cat2 = "availSymp";
    } else if (formID === "gSoln") {
        text = "No solution has been added yet, use the add solution below to add one.";
        location = "#availSoln";
        cat = "solnToAdd";
        cat2 = "availSoln";
    }
    $.post("php/scripts.php", {
        formID: formID
    }, function(info) {
        if ((info.search("0 results")) >= 0) {
            ...
            for (i = 0; i < arrs.length; i++) {
                if (arrs[i] !== "") {
                    ...
                    var availSContF;
                    availSContF = availSCont.replace("%description%", postObj.description);
...
function addRequestToDB(formID) {
    ...
    $.post($("#" + formID).attr("action"), {
        ...
        if (info.includes("Successful")) {
            ...
            $("#" + formID + " input:not(input[type='hidden']):not(input[type='submit'])").each(function() {
                $(this).val("");
            });
            $("#" + formID + " textarea").val("");
            if (formID === "nSympForm") {
...
function showRequestStatus(id) {
    ...
    if (this.hasOwnProperty(id)) {
        $("#" + cat + " ." + cat + ":has(input[value='" + id + "'])").addClass("checked");
    } else {
        $("#" + cat + " ." + cat).has("input[value='" + id + "']").removeClass("checked");
    }
}
...
function showRequestInFault(formID) {
    var text = "",
        ...
        cat = "";
    if (formID === "sSymp") {
        text = "No symptom has been selected yet, use the search symptom above to add one.";
        location = "#chkdSymp";
        obj = sympToAdd;
        cat = "sympToAdd";
        cat2 = "availSymp";
    } else if (formID === "sSoln") {
        text = "No solution has been selected yet, use the search solution above to add one.";
        location = "#chkdSoln";
        obj = solnToAdd;
        cat = "solnToAdd";
        cat2 = "availSoln";
...
function checkRequest(id, avail) {
    if (avail.hasOwnProperty(id)) { return true; } else { return false; }
}
...
function show_a_fault(jqThis) {
    ...
    _aFault = _aFault.replace("%symptom%", gen_cat(pObj.symptoms, sympAvail, sympInAFault));
    _aFault = _aFault.replace("%solution%", gen_cat(pObj.solutions, solnAvail, solnInAFault));
